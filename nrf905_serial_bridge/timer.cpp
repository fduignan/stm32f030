...
{  // Do not call on this in interrupt context
    ...
    if (dly)
    {   
        STK->CVR = STK->RVR; // Must make sure the Systick timer is starting from the beginning
                             // of a period otherwise small delays will be all over the place
                             // This will delay the calling of timed callbacks however
        milliseconds = 0;        
